  ...
  private void registerTorchLauncherItemModel(ItemModelGenerators itemModels) {
    ModelTemplate parentModelTemplate = ModelTemplates.createItem("generated", TextureSlot.LAYER0).extend() //
        ...
        .transform(ItemDisplayContext.THIRD_PERSON_LEFT_HAND,
            builder -> builder.translation(0.25f, 3.125f, 1).scale(0.625f, 0.625f, 0.625f)) //
        .transform(ItemDisplayContext.THIRD_PERSON_RIGHT_HAND,
            builder -> builder.translation(-0.25f, 3.125f, 1).scale(0.625f, 0.625f, 0.625f))
        .build();
    ItemModel.Unbaked modelNotPulling = ItemModelUtils
        .plainModel(itemModels.createFlatItemModel(TorchLauncherModItems.TORCH_LAUNCHER.get(), parentModelTemplate));

    ModelTemplate childModelTemplate = ModelTemplates
        .createItem(TorchLauncherModItems.TORCH_LAUNCHER.getRegisteredName(), TextureSlot.LAYER0);
    ItemModel.Unbaked modelPulling0 = ItemModelUtils.plainModel(
        itemModels.createFlatItemModel(TorchLauncherModItems.TORCH_LAUNCHER.get(), "_pulling_0", childModelTemplate));
    ItemModel.Unbaked modelPulling1 = ItemModelUtils.plainModel(
        itemModels.createFlatItemModel(TorchLauncherModItems.TORCH_LAUNCHER.get(), "_pulling_1", childModelTemplate));
    ItemModel.Unbaked modelPulling2 = ItemModelUtils.plainModel(
        itemModels.createFlatItemModel(TorchLauncherModItems.TORCH_LAUNCHER.get(), "_pulling_2", childModelTemplate));

    itemModels.itemModelOutput.accept(TorchLauncherModItems.TORCH_LAUNCHER.get(),
